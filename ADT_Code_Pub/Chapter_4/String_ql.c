...
Status Replace(String_C *S, String_C T, String_C V){
    int p=0;int k;
    char* tmp="$";
    String_C *S0;StrAssign(S0,tmp);
    Concat(S,*S0,*S);
    int len_T=char_string_length(T);
    int len_V=char_string_length(V);
    if(k=Index(*S,T,p)){
        StrDelete(S,k+1,len_T-1);
        StrInsert(S,k+1,V);
        p=k+len_V-1;
    }
    StrDelete(S,1,1);
...
Status StrInsert(String_C *S, int pos, String_C T){
    ...
    if(pos<=0||pos>len_S)return ERROR;
    if(*S=(String_C)realloc(*S,(len_S+len_T-1)*sizeof(char))){
        memcpy(*S,tmp,pos-1);
        memcpy(*S+pos-1,T,len_T-1);
        memcpy(*S+pos+len_T-2,tmp+pos-1,len_S-pos+1);
        return OK;
    }
    else{return ERROR;}
}//在串S的第pos个字符之前插入串T
Status StrDelete(String_C *S, int pos, int len){
    ...
    for(int n=0;n<len_S;n++){tmp[n]=*(*S+n);}
    if(pos<=0||pos>=len_S)return ERROR;
    if(len<0||len>(len_S-pos))return ERROR;
    if(*S=(String_C)realloc(*S,(len_S-len)*sizeof(char))){
        memcpy(*S,tmp,pos-1);
        memcpy(*S+pos-1,tmp+len+pos-1,len_S-pos-len+1);
        return OK;
    }
    else return ERROR;
}//从串S中删除第pos个字符起长度为len的子串
Status DestoryString(String_C *S){
    if(*S){free(*S);return OK;}
    else return ERROR;
}//串S被销毁
