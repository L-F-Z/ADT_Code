...
//将L重置为空表
Status ListAppend_C(LinkList_C L, ElemType i);
//在L的开头插入元素i
...
Status ListDelete_C(LinkList_C L, int i, ElemType *e)
{
    ...
    int len = ListLength_C(L);
    if (!L || i < 1 || i > len || !L->next)
        return ERROR;
    if (i == 1)
    {
        *e = L->data;
...
void MergeList_C(LinkList_C La, LinkList_C Lb, LinkList_C *Lc, int (*compare)(ElemType, ElemType))
{
    ...
    int cnta=0;
    int cntb=0;
    int lena = ListLength_C(La);
    int lenb = ListLength_C(Lb);
    LinkList_C Lchead=(LinkList_C)malloc(sizeof(struct LNode_C));
    (*Lc) = Lchead;
...
Status ListAppend_C_old(LinkList_C L, ElemType i)
{
    if (!L)
        exit(ERROR);
    if (L->next)
    {
        LinkList_C t = (LinkList_C)malloc(sizeof(struct LNode_C));
        if (!t)
            exit(OVERFLOW);
        t->next = L->next;
        L->next = t;
...
Status ListAppend_C(LinkList_C L, ElemType i)
{
    //注意: 由于特殊的结构定义, 这里的处理与正常插入有所区别, 但是复杂度基本不变
    if (!L)
        exit(ERROR);
    if (L->next)
    {
...
//     CK(LocateElem_C(a, 1, compare));
//     CK(LocateElem_C(a, 2, compare));
//     CK(LocateElem_C(a, 3, compare));
//     CK(LocateElem_C(a, 4, compare));
